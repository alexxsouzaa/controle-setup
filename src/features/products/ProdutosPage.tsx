...
export function ProdutosPage() {
  ...
  const toggleSelectAll = () => {
    ...
    if (paged.every((s: Product) => selected.has(s.id))) setSelected(new Set([...selected].filter(id => !paged.some((s: Product) => s.id === id))));
    else setSelected(new Set([...selected, ...paged.map((s: Product) => s.id)]));
  };
  const clearSelection = () => { setSelected(new Set()); setSelectionMode(false); };
  const selectedCount = selected.size;
  const allSelected = paged.length > 0 && paged.every((s: Product) => selected.has(s.id));
          ...
          <Card>
            ...
            <div className="grid md:grid-cols-3 grid-cols-1 gap-4">
              <div>
                ...
                <Input placeholder="Ex: SHP-400-001" value={form.code} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, code: e.target.value })} />
              </div>
              <div className="md:col-span-2">
                <label className="text-[12px] font-medium text-[var(--fg)] mb-1 block">Nome do produto *</label>
                <Input placeholder="Ex: Shampoo Nutritivo" value={form.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, name: e.target.value })} />
              </div>
            </div>
          </Card>
          <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_120px_1fr] gap-4 mb-4">
              <div>
                ...
                <Input type="number" placeholder="400" value={form.vol} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, vol: e.target.value })} />
              </div>
              <div>
                <label className="text-[12px] font-medium text-[var(--fg)] mb-1 block">Unidade</label>
                <Select value={form.unit} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setForm({ ...form, unit: e.target.value })}><option>ml</option><option>g</option></Select>
              </div>
              <div>
                <label className="text-[12px] font-medium text-[var(--fg)] mb-1 block">Formato</label>
                <Select value={form.formato} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setForm({ ...form, formato: e.target.value })}>

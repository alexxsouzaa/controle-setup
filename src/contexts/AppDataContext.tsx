import { createContext, useState, useCallback, useMemo, useRef, ReactNode } from 'react';
import { Machine, Product, Piece, Flow, Formato, HistoryEntry, Config, Stats, AppActions, AppData } from '../types';
...
const DEFAULT_FORMATOS: Formato[] = [];
const DEFAULT_HISTORY: HistoryEntry[] = [];
const DEFAULT_CONFIG: Config = {
  uoConfigs: {},
};

function loadData(): AppStore {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) {
      const parsed = JSON.parse(raw);
      return {
        machines: (parsed.machines || DEFAULT_MACHINES) as Machine[],
        products: (parsed.products || DEFAULT_PRODUCTS) as Product[],
        pieces: (parsed.pieces || DEFAULT_PIECES) as Piece[],
        flows: (parsed.flows || DEFAULT_FLOWS) as Flow[],
        formatos: (parsed.formatos || DEFAULT_FORMATOS) as Formato[],
        history: (parsed.history || DEFAULT_HISTORY) as HistoryEntry[],
...
const SHAPE = {
  ...
  config: DEFAULT_CONFIG,
  stats: { totalFlows: 0, totalMachines: 0, totalProducts: 0, totalPieces: 0, totalFormatos: 0, activeMachines: 0, flowsToday: 0, lowStockPieces: 0 } as Stats,
  getCurrentUser,
  setCurrentUser,
  updateConfig: ((_updates: Partial<Config>) => {}) as AppData['updateConfig'],
} as AppData;

export const AppDataContext = createContext<AppData>(SHAPE);
...
export function AppDataProvider({ children }: AppDataProviderProps) {
  const [data, setData] = useState<AppStore>(loadData);
  const dataRef = useRef<AppStore>(data);
  dataRef.current = data;

  const save = useCallback((newData: AppStore) => {
  ...
  const actions = useMemo<AppActions>(() => {
    const d = (): AppStore => dataRef.current;
    return {
    addMachine: (m) => {
      ...
      save({ ...d(), machines: [...d().machines, { ...m, lines, toolingCategories, id: uid('mac'), updatedAt: nowISO(), createdAt: m.createdAt || nowISO(), createdBy: user } as Machine] });
    },
    updateMachine: (id, updates) => {
      ...
      save({ ...d(), machines: d().machines.map(mch => mch.id === id ? { ...mch, ...merged, updatedAt: nowISO() } : mch) });
    },
    deleteMachine: (id) => save({ ...d(), machines: d().machines.filter(mch => mch.id !== id) }),
    deleteMachines: (ids) => { const set = new Set(ids); save({ ...d(), machines: d().machines.filter(mch => !set.has(mch.id)) }); },

    addProduct: (p) => save({ ...d(), products: [...d().products, { ...p, id: p.id || uid('prd'), code: p.code, created: p.created || new Date().toISOString().slice(0, 10) } as Product] }),
    updateProduct: (id, updates) => save({ ...d(), products: d().products.map(prd => prd.id === id ? { ...prd, ...updates } : prd) }),
    deleteProduct: (id) => save({ ...d(), products: d().products.filter(prd => prd.id !== id) }),
    deleteProducts: (ids) => { const set = new Set(ids); save({ ...d(), products: d().products.filter(prd => !set.has(prd.id)) }); },

    addPiece: (p) => {
      const compatFromIds = (p.compatibleMachineIds || []).map(id => {
        const mch = d().machines.find(m => m.id === id);
        return mch?.name || '';
      }).filter(Boolean).join(', ');
    ...
    updatePiece: (id, updates) => {
      ...
      const compatFromIds = (updates.compatibleMachineIds || []).map(mid => {
        const mch = machinesList.find(m => m.id === mid);
        return mch?.name || '';
      }).filter(Boolean).join(', ');
      save({ ...d(), pieces: d().pieces.map(pc => pc.id === id ? { ...pc, ...updates, compat: updates.compat || compatFromIds || pc.compat || '' } : pc) });
    },
    deletePiece: (id) => save({ ...d(), pieces: d().pieces.filter(pc => pc.id !== id) }),
    deletePieces: (ids) => { const set = new Set(ids); save({ ...d(), pieces: d().pieces.filter(pc => !set.has(pc.id)) }); },
    ...
    updateFlow: (id, updates) => save({ ...d(), flows: d().flows.map(fl => fl.id === id ? { ...fl, ...updates, updatedBy: updates.updatedBy || getCurrentUser(), updatedAt: nowISO() } : fl) }),
    duplicateFlow: (id) => {
      const flow = d().flows.find(fl => fl.id === id);
      if (!flow) return;
      const existingVersions = d().flows.filter(fl => fl.code === flow.code).length;
      const copy: Flow = { ...flow, id: uid('flow'), date: nowISO(), ver: `V${existingVersions + 1}`, createdBy: getCurrentUser(), createdAt: nowISO(), updatedBy: getCurrentUser(), updatedAt: nowISO() };
      save({ ...d(), flows: [...d().flows, copy] });
    },
    deleteFlow: (id) => save({ ...d(), flows: d().flows.filter(fl => fl.id !== id) }),
    deleteFlows: (ids) => { const set = new Set(ids); save({ ...d(), flows: d().flows.filter(fl => !set.has(fl.id)) }); },
    ...
    addFormato: (f) => {
      ...
      save({ ...d(), formatos: [...d().formatos, {
        ...
        pieces: f.pieces || (f.partIds || []).map(id => {
          const pc = d().pieces.find(p => p.id === id);
          return pc ? { pieceId: pc.id, pieceName: pc.name, pieceCode: pc.code || '', pieceCategory: pc.category || '' } : null;
        }).filter(Boolean),
        createdBy: user,
        createdAt: f.createdAt || nowISO(),
      } as Formato] });
    },
    updateFormato: (id, updates) => save({ ...d(), formatos: d().formatos.map(fmt => fmt.id === id ? { ...fmt, ...updates, updatedBy: getCurrentUser(), updatedAt: nowISO() } : fmt) }),
    deleteFormato: (id) => save({ ...d(), formatos: d().formatos.filter(fmt => fmt.id !== id) }),
    deleteFormatos: (ids) => { const set = new Set(ids); save({ ...d(), formatos: d().formatos.filter(fmt => !set.has(fmt.id)) }); },
    ...
    importData: (imported: unknown): number => {
      ...
      ENTITY_KEYS.forEach(key => {
        ...
        if (Array.isArray(arr) && arr.length > 0) {
          ...
          const newItems = arrTyped.map(item => ({
            ...item,
            id: uid(prefix),
            date: key === 'flows' ? (item.date || new Date().toISOString().slice(0, 10)) : item.date,
            ver: key === 'flows' ? (item.ver || 'v1.0') : item.ver,
          }));
          merged[key] = [...(current[key as keyof AppStore] as unknown[]), ...newItems];
          total += newItems.length;
        }
      });
      if (total > 0) save(merged as unknown as AppStore);
      return total;
    },
  ...
  return (
    <AppDataContext.Provider value={{ ...data, ...actions, stats, getCurrentUser, setCurrentUser } as AppData}>
      {children}

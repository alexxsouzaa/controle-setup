...
function renderBlock(block: PDFBlock): string {
  switch (block.type) {
    ...
    case 'grid-end':
      return `</div></div>`;
    case 'table': {
      const thead = (block.headers ?? []).map((h: string) => `<th>${h}</th>`).join('');
      const tbody = (block.rows || []).map((r: (string | number | null | undefined)[]) =>
        `<tr>${r.map((c: string | number | null | undefined) => `<td>${c ?? '—'}</td>`).join('')}</tr>`
      ).join('');
...
export function buildPDF(title: string, blocks: PDFBlock[]): string {
...
export function printPDF(title: string, blocks: PDFBlock[], toast?: (message: string, type: string) => void): void {

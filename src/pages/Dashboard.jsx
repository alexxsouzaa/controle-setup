...
export function DashboardPage({ navigate }) {
  ...
  const recent = useMemo(() => [...flows].sort((a, b) => (b.date || '').localeCompare(a.date || '')).slice(0, 5), [flows]);
  ...
  return (
    <div className="p-6 pb-16">
      ...
      <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3 mb-6">
        {[
          { label: 'Total de Fluxos', value: stats.totalFlows, icon: 'file', sub: `${today} hoje` },
          { label: 'Máquinas Ativas', value: stats.totalMachines, icon: 'box', sub: 'em operação' },
          { label: 'Setups Hoje', value: today, icon: 'clock', sub: yesterday > 0 ? `${pctChange >= 0 ? '+' : ''}${pctChange}% vs ontem` : 'primeiro do dia' },
          { label: 'Produtos', value: stats.totalProducts, icon: 'grid-3x3', sub: `${stats.totalFormatos} formatos` },
        ].map(s => (
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="border border-[var(--border)] rounded-[8px] overflow-hidden">
          ...
          <div className="p-4 space-y-1">
            {[
              { label: 'Produtos', value: stats.totalProducts, icon: 'grid-3x3', href: '/produtos' },
              { label: 'Peças', value: stats.totalPieces, icon: 'box', href: '/pecas' },
              { label: 'Formatos', value: stats.totalFormatos, icon: 'grid-3x3', href: '/formatos' },
            ].map(item => (
            ...
            <div className="pt-3 mt-3 border-t border-[var(--border)]">
              ...
              <div className="space-y-1">
                {[
                  { label: 'Criar Novo Fluxo', icon: 'wrench', href: '/novo-setup' },
                  { label: 'Importar Dados', icon: 'upload', href: '/importar' },
                  { label: 'Configurações', icon: 'settings', href: '/opcoes' },
                ].map(item => (
                  <button key={item.label} type="button" onClick={() => navigate(item.href)}
          ...
          {recentMachines.length === 0 ? (
            ...
            recentMachines.map(m => (
              <button key={m.id} type="button" onClick={() => navigate('/maquinas')}
                ...
                {m.image ? (
                  <img src={m.image} alt="" className="w-8 h-8 rounded-[6px] object-cover border border-[var(--border)] shrink-0" />
                ) : (

...
export function MaquinasPage({ navigate }) {
  ...
  const toggleLine = (line) => setForm(prev => ({ ...prev, lines: prev.lines.includes(line) ? prev.lines.filter(l => l !== line) : [...prev.lines, line] }));
  const toggleTooling = (cat) => setForm(prev => ({ ...prev, toolingCategories: prev.toolingCategories.includes(cat) ? prev.toolingCategories.filter(c => c !== cat) : [...prev.toolingCategories, cat] }));
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {form.lines.map(l => (
                        <span key={l} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-[4px] border border-[var(--border)] bg-[var(--surface)] text-[11px]">
                          {l}
                          <button type="button" onClick={() => setForm(prev => ({ ...prev, lines: prev.lines.filter(ln => ln !== l) }))} className="text-[var(--fg-muted)] hover:text-[var(--danger)] leading-none">&times;</button>
                        </span>
                      ))}
                    </div>
                  )}
                  ...
                  {lineDropdownOpen && (
                    <div className="absolute z-20 mt-1 w-full bg-[var(--surface)] border border-[var(--border)] rounded-[6px] shadow-md">
                      <div className="p-2 border-b border-[var(--border)]">
                        <div className="flex gap-1">
                          <input className="shad-input flex-1 py-1 text-[11px]" placeholder="Buscar ou criar linha..." value={lineSearch || lineInput} onChange={e => { setLineSearch(e.target.value); setLineInput(e.target.value); }} />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  {toolingDropdownOpen && (
                    <div className="absolute z-20 mt-1 w-full bg-[var(--surface)] border border-[var(--border)] rounded-[6px] shadow-md">
                      <div className="p-2 border-b border-[var(--border)]">
                        <input className="shad-input w-full py-1 text-[11px]" placeholder="Buscar ferramental..." value={toolingSearch} onChange={e => setToolingSearch(e.target.value)} />
                      </div>
                      <div className="max-h-40 overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {getLines(drawerItem).map(l => <Badge key={l}>{l}</Badge>)}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(drawerItem.toolingCategories || []).map(c => <Badge key={c}>{c}</Badge>)}

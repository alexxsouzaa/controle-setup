...
description: "Security audit and vulnerability assessment for any codebase. Scans for authentication bypasses, missing auth middleware, broken JWT validation (algorithm confusion, weak secrets, missing expiry), OAuth state and PKCE flaws, IDOR and horizontal privilege escalation, vertical privilege escalation via role manipulation, SQL injection, NoSQL injection, XSS (stored, reflected, DOM), command injection, path traversal, SSRF, CSRF, hardcoded secrets and API keys (sk_live_, AKIA, ghp_), .env and credential file exposure, PII leaking in logs and error responses, overfetching sensitive fields, CORS misconfiguration, session fixation, missing secure/httpOnly/sameSite cookie flags, Firebase/Firestore rule weaknesses, dependency CVEs (npm/pip/cargo audit), infrastructure misconfig (HTTPS, security headers, Docker, uploads, error leaks), and data protection (mass assignment, input validation, encryption at rest). Produces a severity-ranked findings report with exploit scenarios and fix recommendations. Fixes CRITICAL and HIGH findings when the user asks for remediation. Covers OWASP Top 10. Use when asked to audit, review, or scan a codebase for security."
---
...
- Run the applicable audit tool: `npm audit` / `pip-audit` / `cargo audit` / `bundle audit`.
- Check for known CVEs in major dependencies.
- Flag outdated packages with known security patches.
- Verify a lockfile exists and is committed (supply-chain protection).

============================================================
PHASE 9: CONFIGURATION & INFRASTRUCTURE
============================================================

- HTTPS: verify TLS is enforced (HSTS header, HTTP → HTTPS redirect).
- Security headers: X-Frame-Options, X-Content-Type-Options, Referrer-Policy; flag `unsafe-inline`/`unsafe-eval` in production CSP.
- Error handling: stack traces and internal paths must not leak to clients in production.
- File uploads: size limits, type validation, storage outside webroot.
- Docker/containers: running as root, unnecessary capabilities, secrets baked into the image.
- Logging: sensitive data (passwords, tokens, PII) must not be logged.

============================================================
PHASE 10: DATA PROTECTION
============================================================

- Mass assignment: flag `req.body` spread directly into DB creates/updates.
- Input validation: verify all user input is validated at the boundary (Zod, Joi, pydantic).
- Encryption at rest: sensitive fields (tokens, keys) encrypted in the DB.
- PII exposure: API responses must not include unnecessary fields (email, phone, address).
...
1. **{VULN-001}: {title}**
   - Category: {Authentication/Authorization/Injection/Exposure/Secrets/Session/Dependencies/Infrastructure}
   - Location: `{file:line}`
